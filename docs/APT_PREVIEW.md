...
Schema 8 adds job payload and preview result fields, preserving existing jobs and policy data. Local receipt schema 2 adds job kind and a generic result digest, preserving older inventory digests and receipts. Keep pre-upgrade database and local journal backups for older-version rollback. Later execution-core source advances the local journal to schema 3 for full mutation results. These schemas are unreleased source after 0.6; do not point older binaries at migrated data. The Linux service's existing unsigned read-only compatibility remains; signing/trust deployment must still be provisioned explicitly.


## Artifact binding and locked revalidation

New previews use plan version 2. Every package to install/change has a repository SHA-256 and archive size bound into the plan digest, including dependencies. Missing or ambiguous hashes, mismatched package/version identity and archives above 1 GiB are refused. Version-1 plans remain readable as historical previews but cannot pass locked execution preflight. Repository checksums describe expected bytes; previewing does not download or install them.

`protec.apt_revalidate.locked_plan` is a local, root-only prerequisite for a future executor. It requires native `python3-apt`, acquires `apt_pkg.SystemLock`, compares the dpkg-status digest, resolves the exact request, and refuses any difference in package actions, before/after versions or artifact checksums/sizes. The cache is yielded only inside that lock context and closed on exit. `inspect_plan` never commits. Its returned check result is not a future execution authorization after locks are released.

The disposable Ubuntu guest passed lock exclusion against a competing process, rejection of a changed artifact, unchanged dpkg state and the full HTTP preview/receipt workflow. Python-apt's native locking/commit behavior was inspected on that guest and checked against the official [APT package-system bindings](https://apt-team.pages.debian.net/python-apt/library/apt_pkg.html) and [Cache API](https://apt-team.pages.debian.net/python-apt/library/apt.cache.html). Protected approval, endpoint allowlists, execution with those locks, artifact download verification and non-repeatable mutation recovery are still required before enabling package changes.
