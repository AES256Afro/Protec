...
| M2 | Durable platform data: schema migrations, database backup/restore, health checks, pagination, retention | M0 | Upgrade from existing database, restore drill, full-history counts, controlled retention. |
| M3 | Identity and secrets: SSO/MFA, admin/operator/viewer roles, scoped service tokens, expiry, rotation, device identity, protected secret storage | M1, M2 | Role/target denials, lost-response rotation recovery, expiry and revocation tests; no secrets in logs/UI lists. |
...
| M6 | Package management: installed inventory, repository/catalog sources, package versions, install/update/remove/pin plans, OS adapters | M4, M5 | Inventory matches host; preview shows exact package changes; install/remove exercised on disposable Linux target. APT first, DNF next, Homebrew and winget later. |
