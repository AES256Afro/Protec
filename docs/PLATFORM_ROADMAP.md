...
Status verified September 9, 2026: the private portal and matching public demo run the 0.4.0 inventory pilot; 0.6.0 is published with managed rollout pending. Main also contains the Linux service, static policies and remote APT preview work; these do not establish package execution or full platform completion. See WORK_SESSION.md for exact source, image, test and deployment evidence. Existing work contributes to several phases below, but does not establish completion of those phases. This document describes the long-term destination: proposed architecture, licensing and external-program choices are not accepted decisions merely because they appear here. No license file is present; licensing remains an explicit decision.
