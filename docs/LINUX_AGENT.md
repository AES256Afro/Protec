...
Source after 0.6 first added local receipt schema 2 for package preview results, then schema 3 for durable mutation results. Older agent code may refuse that journal after a migration. The installer now compares receipt schema before an update and after failed activation. If it changed or cannot be read, it stops the service and retains the new code and receipts rather than restarting incompatible older code. Compatible forward recovery was tested in the disposable guest. Same-schema failures retain the existing code/unit fallback; the installer does not restore or merge receipt state. Before a cross-schema service upgrade, preserve a stopped, consistent copy of the enrollment state and journal and test the actual older-code recovery path. Automatic receipt merging or backup restoration remains outside this installer, especially for uncertain package mutations. Do not treat the earlier same-schema rollback test as proof of cross-schema recovery. No real managed agent has been upgraded to this schema in the recorded rollout.
