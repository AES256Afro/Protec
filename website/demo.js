...
(() => {
  ...
  globalThis.protecDemo={async request(path,body){
    ...
    if(body===undefined) {
      ...
      const plan={version:1,kind:'apt_preview',device:device.id,request,changes,created:now(),expires:now()+900,root_simulation:false,requires_revalidation:true,plan_sha256:'a'.repeat(64)};
